...
function Registration() {
  ...
  function getSections() {
    ...
    axios
      .get(
        `http://localhost:80/Prototype-Vite/my-project/api/sectionAvailable/${gradeLevel}`
      )
      .then(function (response) {
  ...
  const handleFileUpload = e => {
    ...
    reader.onload = e => {
      ...
      if (temporary.length == requiredLengthArray) {
        for (let i = 0; i < temporary.length; i++) {
          ...
          if (currentData !== null && currentData !== undefined) {
            ...
            if (userData.length < 7) {
              ...
              rowEmails.push(email + '@Row_.' + i);
              setEmailsArray(rowEmails);
              //console.log(emails);
        ...
        if (isFinished) {
          //console.log(rowEmails);
          const promises1 = rowEmails.map(id =>
            axios.get(
              `http://localhost:80/Prototype-Vite/my-project/api/verifyEmailBulk/${id}`
            )
          );
          Promise.all([...promises1])
            .then(function (result) {
              ...
              let newArray = [];
              for (let i = 0; i < rowEmails.length; i++) {
                let tempArray = [];
                let result = Object.keys(datas[i]).map(key => [
                  key,
                  datas[i][key],
                ]);
                ...
                if (data == 'unique') {
                  ...
                  var tempData = parseInt(data.substring(hashIndex + 1));
                  //console.log(tempData);
                  tempData++;
                  tempData = 'Row #' + tempData.toString();
                  ...
                  setErrorAccount(oldArray => [...oldArray, data]);

                  newArray.push(data);

                  rowErrorDuplicate.push(tempData);
              ...
              //setErrorAccount(rowErrorDuplicate);
  ...
  const bulkRegister = () => {
    ...
    axios
      .post(
        `http://localhost:80/Prototype-Vite/my-project/api/registerBulk/${accounts}`
      )
      .then(function (response) {
        setShowLoading(false);
        setShowModal4(true);
